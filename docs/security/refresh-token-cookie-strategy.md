...
## Gerenciamento centralizado do ciclo de vida do refresh token

Para aplicar essa estratégia diretamente no backend sem duplicar lógica, o repositório disponibiliza a classe `RefreshSessionManager` em `server/security/refresh-session-manager.ts`. Ela encapsula a geração do cookie mascarado, persistência do hash e validação/rotação usando metadados do dispositivo (user-agent e IP, quando fornecidos).

```ts
import { RefreshSessionManager } from '../../server/security/refresh-session-manager'

const sessionManager = new RefreshSessionManager({
  encryptionKey: process.env.REFRESH_TOKEN_ENCRYPTION_KEY!,
  tokenStore: {
    async save(record) {
...
// Emite o cookie durante o login
const { cookie, tokenHash } = await sessionManager.issueSession({
  userId: usuario.id,
  metadata: {
...
// Valida e rotaciona durante o refresh
...
if (!validation.valid) throw new UnauthorizedError()
...
if (rotation.rotated) {
  reply.header('Set-Cookie', rotation.cookie)
}

// Durante logout, remova o registro e invalide o cookie
await sessionManager.revokeSessionFromCookie({
  cookieHeader: request.headers.cookie,
  metadata: {
    userAgent: request.headers['user-agent'] ?? undefined,
    ipAddress: realIp,
...
reply.header('Set-Cookie', sessionManager.buildRevocationCookie())
```

Além dos métodos de emissão e validação, o gerenciador oferece `revokeSessionFromCookie` para cenários de logout utilizando apenas o cabeçalho `Cookie`, bem como `buildRevocationCookie` para enviar um cookie expirado com as mesmas flags endurecidas.
